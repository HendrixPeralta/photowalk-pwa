...
export async function initStorage() {
  if (pendingImageMigration.length) {
    try {
      ...
      showToast("Couldn't update your saved photos. They're still safe, and PhotoEYE will try again next time.", 6000);
    }
  }
  requestPersistence();
}
...
export async function warnIfStorageTight() {
  ...
  lastQuotaWarning = Date.now();
  showToast(`Storage is ${Math.round(info.ratio * 100)}% full. Try deleting some older references.`, 6000);
}
